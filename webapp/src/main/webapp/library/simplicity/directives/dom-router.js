...
class DomRouter extends HTMLElement {
    ...
    destroy() {
        // Resolve is called because the Proxy is everytime new
        window.removeEventListener("popstate", this.handler.resolve);
    }

    initialize() {
        this.style.display = "block"
        // Resolve is called because the Proxy is everytime new
        window.addEventListener("popstate", this.handler.resolve);
        this.handler();
    }
