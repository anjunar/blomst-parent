...
public class StatusProvider extends AbstractRestPredicateProvider<Status, CommunitiesConnection> {
    @Override
    public Predicate build(Status value, IdentityManager identityManager, EntityManager entityManager, CriteriaBuilder builder, Root<CommunitiesConnection> root, CriteriaQuery<?> query, BeanProperty<?, ?> property) {
        if (value == null) {
